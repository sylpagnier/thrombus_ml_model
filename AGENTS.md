...
- **GNODE-ODE ladder (9.x, GT vel)**: doc [src/docs/GNODE_ODE_LADDER.md](src/docs/GNODE_ODE_LADDER.md); smoke **9.1** `go_gnode91_smoke.ps1`; ~8h queue **9.4-9.6** `go_gnode_8h_ladder.ps1`; headless teacher PNG `snapshot_biochem_teacher.py` + clot-band `snapshot_biochem_teacher_clotband.py`.
...
- **Production finetune** (continuity + L2, from best.pth): `powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_kinematics_production_allfix_finetune.ps1" -ContinuityFocus`
