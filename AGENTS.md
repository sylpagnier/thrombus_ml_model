...
- Latest (2026-06-21): **relative loss** (`--loss relative`, per-patch energy-normalized; floor `0.5*median`, grad_clip 5) 800 ep -> global relL2 **15.9%**, val MSE 1.06e-6, beating the MSE baseline (17.6% / 1.30e-6). Tail diagnosis showed the error lived in the *low-signal* (low-shear/thin) patches, not the hard clots; the relative loss fixed that and improved the global metric too. NOTE: a naive `1e-12` denominator floor collapses the model (val relL2~100%) -- must floor at a fraction of median patch energy. Bucket table confirmed it flattened the tail (p90 42->29%, max 98->66%; low terciles all dropped); only hot cell left is lowest-shear (24%). See doc run log.
