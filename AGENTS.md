...
- **K11e** (localized clot gate, best-practice): `powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_k11e_clot_gate.ps1" -Fresh` (alias: `go_k11_clot_gate.ps1`). Sets `BIOCHEM_PRIOR_COMSOL_ALIGNED=1` (hybrid stream + **dγ/dx** gate, adjacent p95 norm; no `K11_MECH_QUANTILE`). Viz: `…/biochem_teacher_best_high_mu.pth` (K11 ckpt score). Prior diagnostic: `python scripts/diagnose_k11_clot_prior.py --anchor patient007 --checkpoint outputs/biochem/biochem_teacher_best_high_mu.pth`. Kinematics fields: `src/core_physics/clot_kinematics_fields.py`; pattern test: `src/tests/test_clot_node_pattern.py`.
...
- **Recovery sweep ~12h** (main `graphs_kinematics/newtonian`, 8 recipes → `sweep_recovery_12h/`): `powershell -File .\scripts\go_kinematics_recovery12h.ps1`
