...
- **Mat-growth canonical baseline (W vs WC):** **WC_v2_dilation** promoted 2026-07-06 (evaluation environment fixes, off-wall clot predictions enabled); ckpt `outputs/biochem/biochem_gnn/mat_canonical_deploy/species/best.pth`; alias `mat_canonical_deploy/species/best.pth` via `-Promote` or custom copy; viz `viz_pivot3_hop_analysis.py` (dilation vs baseline); doc [docs/MAT_GROWTH_SIM_TODO.md](docs/MAT_GROWTH_SIM_TODO.md). Legacy baseline: `WC_mat_flow_dynamic` (wall-only, June).
- **Off-wall clot growth pivot sweep (2026-07-05, §194):** `scripts/go_off_wall_clot_sweep_6h.ps1` — 5 legs (Pivot 1 skip-hop / Pivot 2 shear-gate / **Pivot 3 occlusion** / Pivot 4 frontier / combined) vs `WC_mat_3hop`. **Verdict:** only **Pivot 3** (`BIOCHEM_ROLLOUT_DYNAMIC_OCCLUSION=1`) survives — rest clot F1 **0.804**, score **0.988** (+0.019 vs baseline); Pivots 1/2/4 degrade ≥40%. Next: fold Pivot 3 into `WC_mat_3hop` as `WC_canonical_v2`. Best ckpt: `mat_growth_ladder/WC_pivot3_occlusion/species/best.pth`.
...
- Latest (2026-06-21): **relative loss** (`--loss relative`, per-patch energy-normalized; floor `0.5*median`, grad_clip 5) 800 ep -> global relL2 **15.9%**, val MSE 1.06e-6, beating the MSE baseline (17.6% / 1.30e-6). Tail diagnosis showed the error lived in the *low-signal* (low-shear/thin) patches, not the hard clots; the relative loss fixed that and improved the global metric too. NOTE: a naive `1e-12` denominator floor collapses the model (val relL2~100%) -- must floor at a fraction of median patch energy. Run `eval_local_corrector` for the bucket table. See doc run log.
...
## Checkpoint Evaluation and Environment Mismatch (Off-wall metrics)

- **Checkpoint Env Preservation:** In HemoGINO, biochem models use dynamic physical environment overrides (`env_overrides` like `CLOT_PHI_PHYSICS_WALL_MAT_ONLY=0` and `CLOT_V2_NUCLEATION_HOPS=3`). Evaluating checkpoints without loading their corresponding leg overrides leads to incorrect masking (e.g. zeroing out off-wall predictions).
  - Always save `leg` and `env_overrides` in checkpoint metadata (`meta`) when training.
  - When loading checkpoints for evaluation or rollouts (e.g. in `species_gnn_clot_rollout.py` or viz scripts), explicitly restore environment variables using `meta.get("env_overrides")` or infer and apply the leg specs from the checkpoint file path.
  - Evaluate off-wall metrics strictly on nodes at Hop $\ge$ 1 using `deploy_clot_offwall_relaxed_f1` and related indicators.

## Hardware Execution Policy

- **Strict CUDA Enforcement:** Silent fallbacks to CPU during training loops can cause processes to hang or run extremely slowly. Always enforce `require_cuda_device()` at the entry points of training scripts (`train_species_pushforward_continuous.py`, `train_species_snapshot_gnn.py`), failing out loud with a `RuntimeError` if CUDA is unavailable.
