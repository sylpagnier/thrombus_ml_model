...
- **M3 ADR alignment block** (Phase B ramp + cal probe + narrowing + sweep + lock): `powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_m3_block_pass.ps1"` (`-Turbo` ~3-5h; prereq `biochem_teacher_passive_species_locked.pth`). Re-verify: `python scripts/check_m3_block_pass.py`.
