...
- **Clot forecast ladder (R0–R6, fresh start)**: [docs/CLOT_FORECAST_LADDER.md](docs/CLOT_FORECAST_LADDER.md) — R0 label sanity → R1 one-step (`go_clot_forecast_r1.ps1` A–D, **promoted R1D** `deploy_pred`) → **R2** carry (`go_clot_forecast_r2.ps1`, R1D init) → R4 GINO-DEQ → R6 deploy. **Viz:** metrics at R1; R2+ PNG + **`clot_shape`** in logs.
