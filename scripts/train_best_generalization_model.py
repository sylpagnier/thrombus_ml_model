...
def run_stage(
    ...
    for epoch in range(epochs):
        ...
        print(f"  Epoch {epoch+1:02d}/{epochs} - Score: {score:.4f}")
        ...
        if epochs_without_improvement >= patience:
            print(f"[i] Early stopping triggered at epoch {epoch+1}.")
            break
...
def train_best_generalization_model():
    # Winning Hyperparameters from the 2-Hour Sweep
    hidden_channels = 64
    num_layers = 2
    dice_weight = 9.82364264801026
    focal_weight = 4.743830569945526
    spatial_lr = 0.00020092515070008042
    temporal_lr = 3.441309398206217e-05
    joint_lr = 7.5162245505065075e-06
    
    save_path = Path("outputs/biochem/best_generalization_model.pth")
    save_path.parent.mkdir(parents=True, exist_ok=True)
    
    print("\n[i] Retraining best generalization Optuna model to save weights...")
    ...
    print(f"\n[OK] Retraining Complete. Weights saved to {save_path}")

if __name__ == "__main__":
    train_best_generalization_model()
