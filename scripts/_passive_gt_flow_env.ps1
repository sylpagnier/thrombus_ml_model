# Shared passive-transport env (GT COMSOL flow; no Stage-A kin checkpoint required).
# Dot-source from launchers: . (Join-Path $PSScriptRoot "_passive_gt_flow_env.ps1")

function Set-PassiveGtFlowEnv {
    param(
        [string] $RunNote,
        [switch] $ClotBandMask,
        [switch] $AdrBackprop,
        [string] $AdrWeight = "1e-3"
    )
    ...
    if ($ClotBandMask) {
        ...
        $env:BIOCHEM_DATA_BIO_MASK_MODE = "global"
    }
