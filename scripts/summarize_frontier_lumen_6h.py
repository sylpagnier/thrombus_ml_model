"""Summarize FrontierLumen 6h scale-up: Arm A (canonical) vs Arm S (compound).
...
def main() -> int:
    ...
    ap.add_argument("--wall-clot-floor-delta", type=float, default=0.03)
    ap.add_argument("--out", default="")
    args = ap.parse_args()

    a = _load(args.arm_a, "A_canonical_WC_v7")
    s = _load(args.arm_s, "S_compound_frontier_lumen")
    delta = {
    ...
    if wall_ok and hop_vol_up and hop_strict_up:
        verdict = "pass_scale_signal"
    ...
    print("=" * 80, flush=True)
    print("WC_v7 FRONTIER_LUMEN 6H  A vs S", flush=True)
    print("=" * 80, flush=True)
    print(
        f"{'metric':<40} {'A canon':>10} {'S lumen':>10} {'dS-A':>10}",
        flush=True,
    )
    for k in METRICS:
        print(
    ...
    print(
        ...
        f"hop_strict_up={hop_strict_up} -> verdict={verdict}",
        flush=True,
    )
