param(
    ...
    [string] $RunRoot        = "outputs/biochem/eda/wall_gen_prec_seed_aux",
    [string] $InitCkpt       = "outputs/biochem/eda/wall_gen_prec_iter/WG_prec_iter/best.pth",
    [string] $Leg            = "WG_prec_seed_aux",
    [switch] $Fresh,
    [switch] $EvalOnly,
    [switch] $SkipViz,
    [switch] $NoInit
...
# Early seed-location aux on prec-iter (no hard frontier). Warm-start WG_prec_iter.
#   .\scripts\go_wg_prec_seed_aux.ps1 -Epochs 25 -EarlyStop 8 -Fresh
...
if ($Leg -ne "WG_prec_seed_aux") {
    Write-Host "[ERR] This launcher is for WG_prec_seed_aux only (got $Leg)" -ForegroundColor Red
    exit 1
}
...
if ($trainList.Count -lt 2 -or $trainList.Count -gt 8) {
    Write-Host "[ERR] seed-aux expects 2-8 train vessels, got $($trainList.Count)" -ForegroundColor Red
    exit 1
}
...
Write-Host "[NEW] wg_prec_seed_aux ($Leg): $Epochs ep / ES $EarlyStop / lr=$Lr" -ForegroundColor Cyan
Write-Host "[i] goal=early seed LOCATION aux (not hard frontier; not another mass term)" -ForegroundColor DarkGray
Write-Host "[i] stack=WG_prec_iter + seed_aux_weight=0.15 early_steps=3 compact=0.05; fh=0 tk=0" -ForegroundColor DarkGray
Write-Host "[i] select=clot score + seed_prec/front/FN-FP panel on $HoldoutAnchor" -ForegroundColor DarkGray
