# R2: multi-step clot rollout (GT flow + carry), forecast-ladder aligned with R1D.
#
...
# Deploy neighbor commit band (pred phi + carried mu seeds)
...
$env:CLOT_PHI_SWEEP_LEG = "r2_rollout_gt"

$LegDir = Join-Path $RepoRoot "outputs/biochem/clot_forecast_ladder/r2_rollout_gt"
...
Write-Host "[NEW] R2 forecast rollout (GT vel + carry log_mu, deploy_pred band) epochs=$Epochs" -ForegroundColor Cyan
Write-Host "[i]  mask=$env:CLOT_FORECAST_MASK carry_log_mu=1 carry_phi=0 init=$($env:CLOT_PHI_INIT_CHECKPOINT)" -ForegroundColor DarkGray
...
$VizOut = Join-Path $RepoRoot "outputs/biochem/clot_forecast_ladder/viz/r2_rollout_gt_patient007_tfinal.png"
...
Write-Host "[OK]  R2 done ckpt=$Ckpt" -ForegroundColor Green
