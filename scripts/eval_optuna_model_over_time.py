...
def evaluate_over_time():
    ...
    val_vessels = ["patient020", "patient034", "patient015", "patient016"]
    
    # 1. Build Model with Optimal Hyperparameters
    hidden_channels = 64
    num_layers = 2
    ...
    first_data_path = Path("data/processed/graphs_biochem_anchors/patient020.pt")
    ...
    for vessel in val_vessels:
        data_path = Path(f"data/processed/graphs_biochem_anchors/{vessel}.pt")
        if not data_path.exists():
            continue
            
        data = torch.load(data_path, map_location="cpu", weights_only=False).to(device)
        t_max = int(data.y.shape[0])
        print(f"\n[i] Evaluating {vessel} over {t_max} time steps...")
        ...
        with torch.no_grad():
            ...
            for t_idx in range(t_max):
                ...
                # Enforce no clots at t=0
                if t_idx == 0 or float(data.t[t_idx]) <= 1e-6:
                    final_prob_t = torch.zeros_like(final_prob_t)
                
                # Target
                target_t = gt_clot_phi_at_time(data, t_idx, phys_cfg, device=device).reshape(-1)
                target_t = target_t * wall_mask
                
                # Calculate True Deploy Score
                ...
                scores_for_vessel.append(deploy_score)
                
        all_vessel_scores.append(scores_for_vessel)
        avg_score = np.mean(scores_for_vessel)
        print(f"  -> Mean Deploy Score for {vessel}: {avg_score:.4f}")
        
    # Calculate overall mean across all vessels and times
    flat_scores = [s for vessel_scores in all_vessel_scores for s in vessel_scores]
    overall_mean = np.mean(flat_scores)
    print(f"\n[OK] Overall Mean Deploy Score across {len(val_vessels)} validation vessels over time: {overall_mean:.4f}")
