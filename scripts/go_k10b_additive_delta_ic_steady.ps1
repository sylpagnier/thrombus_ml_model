# K10b: K10a + split Δμ heads + additive bulk/tail mixing (forward_policy saved in .pth for viz).
# Prereq: outputs/kinematics/kinematics_best.pth. Delete outputs/biochem/*.pth for fresh run.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_k10b_additive_delta_ic_steady.ps1" -Fresh
...
$env:BIOCHEM_RUN_NOTE = "K10b_additive_delta_ic_steady"
...
if ($Fresh) {
    ...
    Write-Host "Fresh K10b: deleted outputs/biochem/*.pth (kept kinematics_best.pth)." -ForegroundColor Cyan
}

$pyArgs = @(
    "-m", "src.training.train_biochem_corrector",
    "--new", "--epochs", "12", "--save-best",
    "--run-name", "K10b_additive_delta_ic_steady"
)
