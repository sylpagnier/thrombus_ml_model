...
| GNODE 9.x (GT vel) | `go_gnode91_smoke.ps1` (9.1); `go_gnode_8h_ladder.ps1` (9.4-9.6 ~8h); `go_gnode99.ps1` (9.9 best-practice clot_band); `snapshot_biochem_teacher*.py`; `_gnode_viz_helpers.ps1` — [GNODE_ODE_LADDER.md](../docs/GNODE_ODE_LADDER.md) |
...
| **Clot baseline (deploy)** | **`go_baseline_clot.ps1`** (train + promote); **`go_baseline_clot_viz.ps1`**; `promote_clot_baseline.py`; `python -m src.inference`; artifacts under `outputs/biochem/clot_baseline/` |
| **CAVO deploy ladder** | **`go_clot_deploy_ladder.ps1`** (rule S0-G2 gates); **`go_clot_deploy_rule_timeline_viz.ps1`** (phi over time); **`go_clot_deploy_s0_rule_viz.ps1`** (tfinal snapshot); **`go_clot_deploy_phase1.ps1`** (G1 MLP); doc [DEPLOY_ARCHITECTURE.md](../docs/DEPLOY_ARCHITECTURE.md) |
| **Mu coupling A/B/C** | **`go_mlp_abc_scorecard.ps1`** (north-star clot shape + flow guards); **`go_mlp_abc_viz.ps1`** (interactive or `-Headless` PNGs); **`go_mlp_abc_compare_1h.ps1`** (legacy JSON path); **`go_mlp_mu_map_v2_viz.ps1`** (Leg B only); `abc_scorecard*.json` |
| Sweeps | `go_health10h`, `go_visc3h`, **`go_mu_complexity_6h.ps1`** (~6h **comprehensive** pred-kine + teacher->corrector/synth; `summarize_mu_complexity_6h.py`) |
...
| Script | Notes |
|--------|--------|
| `src.tools.extract_biochem_comsol` | After manual COMSOL export: pick stem, see extract status, write `graphs_biochem_anchors/*.pt` (`go_extract_biochem_interactive.ps1`, `python -m src.bin.main data extract-biochem`) |
| `reextract_anchor_comsol_graphs.py` | Batch re-extract all (or `--stem`) with optional sidecar enrich |
...
| `summarize_mu_complexity_6h.py` | Rank `sweep_mu_complexity_6h` legs by val `mu_log_mae` (best epoch) |
