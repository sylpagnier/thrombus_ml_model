"""T1 clot trigger timeline viz: physics | hybrid (deploy rollout) | GT phi."""
...
from src.core_physics.clot_trigger_rollout import rollout_clot_trigger_hybrid
...
from src.training.clot_trigger_stack import apply_clot_trigger_deploy_env
...
def main() -> int:
    ap = argparse.ArgumentParser(description="T1 clot trigger timeline viz")
    ap.add_argument("--anchor", default="patient007")
    ap.add_argument("--anchor-dir", default="data/processed/graphs_biochem_anchors")
    ap.add_argument(
        "--checkpoint",
        default="outputs/biochem/clot_trigger/t1/clot_trigger_t1_best.pth",
    )
    ...
    os.environ.setdefault("CLOT_PHI_PHYSICS_BLEND", "1")
    os.environ.setdefault("CLOT_PHI_SPECIES_FEATURES", "1")
    ...
    hybrid_traj = rollout_clot_trigger_hybrid(
        model,
        data,
        phys_cfg=phys,
        bio_cfg=bio,
    ...
    for t in times:
        ...
        phi_hybrid_deploy = hybrid_traj[int(t)]["phi"].reshape(-1)
        m_phys = clot_trigger_viz_f1(display["phi_phys"], display["phi_gt"], mask)
        m_hyb = clot_trigger_viz_f1(phi_hybrid_deploy, display["phi_gt"], mask)
        frames.append(
            {
                "t": int(t),
                ...
                "phi_hybrid": phi_hybrid_deploy.detach().cpu().numpy(),
    ...
    fig.suptitle(
        f"T1 clot trigger -- {args.anchor} | ceiling band F1 | row0=GT | row1=physics | row2=hybrid rollout",
        fontsize=11,
    )
    row_labels = ("GT", "physics (GT kine)", "hybrid (deploy rollout)")
    ...
    out_path = Path(args.out) if args.out.strip() else (
        root / f"outputs/biochem/viz/clot_trigger/t1_{args.anchor}.png"
    )
    ...
    summary_path.write_text(
        json.dumps(
            {
                "anchor": args.anchor,
                "checkpoint": str(ckpt_path.relative_to(root) if ckpt_path.is_relative_to(root) else ckpt_path),
                "frames": [{k: v for k, v in fr.items() if k not in ("phi_gt", "phi_phys", "phi_hybrid", "region")} for fr in frames],
            },
            indent=2,
        ),
        encoding="utf-8",
