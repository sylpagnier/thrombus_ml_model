# Phase 4: long-horizon stabilization (noisy teacher + TBPTT curriculum + velocity decay).
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_species_snapshot_s32.ps1" -Fresh -AllAnchors
...
param(
    ...
    [int] $Unroll = 10,
    [int] $Epochs = 50,
    [string] $Ckpt = "outputs/biochem/species_snapshot_s32/best.pth",
    [string] $InitS31 = "outputs/biochem/species_snapshot_s31/best.pth",
    [switch] $SkipTrain,
    [switch] $Fresh,
    [switch] $VizOnly,
    [int] $EarlyStop = 35
...
if (-not $SkipTrain -and -not $VizOnly) {
    Write-Host "[NEW] Train species s32 (long-horizon stabilization)" -ForegroundColor Cyan
    $trainArgs = @(
    ...
    Invoke-PythonRcCheck -Label "species s32 train" -PyArgs $trainArgs
}

if (-not $SkipTrain -or $VizOnly) {
    Write-Host "[NEW] Viz s32 species ladder" -ForegroundColor Cyan
    Invoke-PythonRcCheck -Label "species s32 species ladder" -PyArgs @(
        "scripts/viz_species_gnn_species_ladder.py",
        "--anchor", $ValAnchor,
        "--ckpt", $Ckpt
    )
    Invoke-PythonRcCheck -Label "species s32 multi-anchor eval" -PyArgs @(
        "scripts/eval_species_gnn_multi_anchor.py", "--ckpt", $Ckpt
    )
    Invoke-PythonRcCheck -Label "species s32 clot ladder" -PyArgs @(
