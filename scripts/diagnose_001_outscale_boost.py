"""Ablate SPECIES_CONTINUOUS_DELTA_OUT_SCALE boost on band mini-train + deploy."""
...
def main() -> int:
    ...
    os.environ["SPECIES_CONTINUOUS_DELTA_OUT_SCALE"] = "5e-4"
    ...
    print(f"[i] out_scale override {continuous_delta_out_scale()}", flush=True)
    ...
    _mini_train(model, tile, steps=80, lr=3e-4, lumen_w=10.0, device=device)
    model.eval()
    ...
    print(
        f"[i] boost fire {pre['n_fire_gt_thr']:.0f}->{post['n_fire_gt_thr']:.0f} "
    ...
    tmp = out_dir / "_narrow_growth_band_boost.pth"
    save_continuous_checkpoint(
        tmp, model, {"out_scale": "5e-4", "train_feat_source": "band"}
    )
    ...
    print("[i] boosted compound deploy...", flush=True)
    ...
    print(f"[OK] boost hop_ge2={ge2:.0f}/{gt:.0f} clot_f1={f1:.3f}", flush=True)
    (out_dir / "diagnose_narrow_boost.json").write_text(
        json.dumps(
            {
    ...
    print(f"[save] {out_dir / 'diagnose_narrow_boost.json'}", flush=True)
    return 0
