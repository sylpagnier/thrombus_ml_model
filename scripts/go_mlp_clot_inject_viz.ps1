# Viz with MLP clot-trigger mu injection enabled during GNODE rollout (closed-loop).
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\go_mlp_clot_inject_viz.ps1
#   powershell ... -Anchor patient007 -Blend 0.75
...
$env:BIOCHEM_MLP_CLOT_INJECT = "1"
$env:BIOCHEM_MLP_CLOT_CKPT = $ClotPhi
$env:BIOCHEM_MLP_CLOT_MU_SI = "$MuClotSi"
$env:BIOCHEM_MLP_CLOT_BLEND = "$Blend"
$env:BIOCHEM_MLP_CLOT_REGION = $Region

Write-Host "[NEW] Teacher viz + MLP clot mu inject" -ForegroundColor Cyan
Write-Host "[i]  inject mu_clot=${MuClotSi} blend=$Blend region=$Region" -ForegroundColor DarkGray
