param(
    ...
    [string] $RunRoot        = "outputs/biochem/eda/wall_gen_prec_iter",
    [string] $InitCkpt       = "outputs/biochem/eda/wall_gen_featfix/WG_featfix_03/best.pth",
    [string] $Leg            = "WG_prec_iter",
    [switch] $Fresh,
    [switch] $EvalOnly,
    [switch] $SkipViz,
    [switch] $NoInit
...
# Small-cohort precision iteration: fix train-deploy objective mismatch before N+.
# Train 005/006/010 only; holdout patient020; warm-start featfix_03; mass-gated select.
#   .\scripts\go_wg_prec_iter.ps1 -Epochs 25 -EarlyStop 8 -Fresh
...
if ($trainList.Count -lt 2 -or $trainList.Count -gt 8) {
    Write-Host "[ERR] prec-iter expects 2-8 train vessels, got $($trainList.Count)" -ForegroundColor Red
    exit 1
}
...
Write-Host "[NEW] wg_prec_iter ($Leg): $Epochs ep / ES $EarlyStop / lr=$Lr" -ForegroundColor Cyan
Write-Host "[i] goal=bind train loss to cold deploy (step+final mass/FP); small cohort before N+" -ForegroundColor DarkGray
Write-Host "[i] stack=featfix_03 + mature_fp_on + teacher_fp=0 + mass-gated select" -ForegroundColor DarkGray
...
Write-Host "[i] success: cold score >= featfix03 (~0.329) with mass <= ~1.5 (not spray)" -ForegroundColor DarkGray
if ($NoInit) {
...
if (-not $SkipViz) {
    ...
    $vizOut = Join-Path $vizDir "clot_ladder_prec_iter_$HoldoutAnchor.png"
