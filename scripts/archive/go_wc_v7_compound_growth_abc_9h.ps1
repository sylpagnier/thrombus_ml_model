# WC_v7 canonical vs compound growth A/B/C.
#
# Arms:
#   A  - frozen canonical WC_v7_clot_phi_mse (locked/species_gnn_best.pth)
#   B  - original compound hypothesis:
#          growth specialist: frontier supervise + loss_blurring
...
#
# Timing (measured 2026-07-20 on 4GB GPU, --all-anchors ~35 graphs):
#   Train one specialist:  ~8-10 h  (~19 min/epoch; ES~30 -> ~9.6 h for Arm B)
#   Train B then C:        ~16-20 h
#   Eval A + B (cohort):   ~2-6 h   (full deploy rollouts; cancel-friendly)
#   Full A/B/C pipeline:   ~20-26 h  (NOT a 9 h job — filename is historical)
#
# Partial run saved:
#   Arm B best -> outputs/biochem/offwall_model/wc_v7_compound_abc_9h/growth_B_frontier_blurring/best.pth
#   Resume A vs B compare (no retrain):
#     powershell ... -File .\scripts\go_wc_v7_compound_growth_abc_9h.ps1 -EvalOnly -SkipC
#
# Faster options:
#   -TrainAnchors "patient001,patient002,..."   # restrict train set
#   -Smoke -Fresh                               # <1 min wiring check
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\go_wc_v7_compound_growth_abc_9h.ps1 -Fresh
...
param(
    ...
    [string] $RunRoot = "outputs/biochem/offwall_model/wc_v7_compound_abc_9h",
    [switch] $Fast,
    [switch] $Smoke,
    [switch] $Fresh,
    [switch] $EvalOnly,
...
if ($Smoke) {
    ...
    $RunRoot = "outputs/biochem/offwall_model/wc_v7_compound_abc_smoke"
    # One compound eval is enough to exercise TWO_MODEL wiring under the time budget.
    $SkipSummary = $true
    Write-Host "[i] SMOKE preset: train B+C (cheap-val) + one wall-route compound eval" -ForegroundColor Yellow
} elseif ($Fast) {
    $Epochs = $FAST_EPOCHS
...
$GrowthB = Join-Path $OutDir "growth_B_frontier_blurring/best.pth"
...
Write-Host "[NEW] go_wc_v7_compound_growth_abc_9h" -ForegroundColor Cyan
...
Write-Host "[i] Arm B: supervise=frontier loss=loss_blurring route=frontier ckpt=clot_score" -ForegroundColor DarkGray
Write-Host "[i] Arm C: supervise=offwall loss=loss_blurring_prec route=wall ckpt=offwall_balanced" -ForegroundColor DarkGray
Write-Host "[i] run_root=$RunRoot fresh=$Fresh smoke=$Smoke evalOnly=$EvalOnly skipB=$SkipB skipC=$SkipC" -ForegroundColor DarkGray
...
function Invoke-GrowthTrain {
    ...
    if ($TrainAnchors.Trim()) {
        $trainArgs += @("--anchors", $TrainAnchors.Trim())
    } else {
...
function Invoke-CompoundEval {
    ...
    if ($EvalAnchors.Trim()) {
        $evalArgs += @("--anchors", $EvalAnchors.Trim())
    }
...
if (-not $EvalOnly) {
    if (-not $SkipB) {
        Write-Host "[i] Phase 1a: train Arm B specialist (frontier + blurring)..." -ForegroundColor Cyan
        Invoke-GrowthTrain -Label "Arm B growth train" -OutCkpt $GrowthB `
            -SuperviseMode "frontier" -LossMode "loss_blurring" `
            -CkptMetric "clot_score" -FrHops $FrontierHops
    } else {
...
# ---------------------------------------------------------------------------
# Smoke: skip canonical-only eval (slow full deploy); still exercise both compound routes.
if (-not $Smoke) {
    ...
    # Minimal stub so summarize still has an Arm A slot if needed later.
    @{
        anchors = @($ValAnchor)
...
Write-Host "[OK] WC_v7 compound A/B/C done -> $OutDir" -ForegroundColor Green
