# Phase 4.5: geometric saturation gate (sat_ratio input + magnitude headroom clamp + mature FP exempt).
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_species_snapshot_s33.ps1" -Fresh -AllAnchors
...
param(
    ...
    [int] $Unroll = 10,
    [int] $Epochs = 50,
    [string] $Ckpt = "outputs/biochem/species_snapshot_s33/best.pth",
    [string] $InitS32 = "outputs/biochem/species_snapshot_s32/best.pth",
    [switch] $SkipTrain,
    [switch] $Fresh,
    [switch] $VizOnly,
    [int] $EarlyStop = 35
...
# s32 recipe + Phase 4.5 saturation gate
...
if (-not $SkipTrain -and -not $VizOnly) {
    Write-Host "[NEW] Train species s33 (saturation gate)" -ForegroundColor Cyan
    $trainArgs = @(
    ...
    Invoke-PythonRcCheck -Label "species s33 train" -PyArgs $trainArgs
}

if (-not $SkipTrain -or $VizOnly) {
    Write-Host "[NEW] Viz s33 species ladder" -ForegroundColor Cyan
    Invoke-PythonRcCheck -Label "species s33 species ladder" -PyArgs @(
        "scripts/viz_species_gnn_species_ladder.py",
        "--anchor", $ValAnchor,
        "--ckpt", $Ckpt
    )
    Invoke-PythonRcCheck -Label "species s33 multi-anchor eval" -PyArgs @(
        "scripts/eval_species_gnn_multi_anchor.py", "--ckpt", $Ckpt
    )
    Invoke-PythonRcCheck -Label "species s33 clot ladder" -PyArgs @(
