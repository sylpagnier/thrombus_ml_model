...
def check_patient015():
    ...
    # We want the final deploy score. Let's just evaluate at t_max - 1
    t_idx = t_max - 1
    ...
    with torch.no_grad():
        ...
        print(f"Metrics for {anchor} at final t={t_idx}:")
        for k, v in d.items():
            print(f"  {k}: {v:.4f}")
        print(f"  --> DEPLOY CLOT SCORE: {deploy_score:.4f}")
        print(f"      (Model TP/FP/FN/TN):")
        print(f"      Predicted Clot nodes: {int(binary_pred.sum())}")
        print(f"      True Clot nodes: {int(target_t.sum())}")
