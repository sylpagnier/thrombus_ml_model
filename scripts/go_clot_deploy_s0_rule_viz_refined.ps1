# Viz refined sweep winner (prior_p0.80 | flux_stag_top20 | tie_dx_hop).
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_clot_deploy_s0_rule_viz_refined.ps1"
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_clot_deploy_s0_rule_viz_refined.ps1" -MaskOverlay
...
$png = Join-Path $vizDir "prior_rule_refined_${Anchor}_${tLabel}_fullmesh.png"
...
if ($MaskOverlay) {
    $overlay = Join-Path $vizDir "prior_rule_refined_${Anchor}_${tLabel}_masks.png"
    $pyArgs += @("--mask-overlay-out", $overlay)
}

Write-Host "[NEW] refined prior rule viz anchor=$Anchor rule=prior_p0.80|flux_stag_top20|tie_dx_hop" -ForegroundColor Cyan
Invoke-PythonRcCheck @pyArgs -Label "refined prior rule viz"
