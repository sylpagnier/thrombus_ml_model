# Short teacher probe: geometry-isolated wall head (wall nodes learn clot viscosity).
# Historical A/B (2026-05-23) showed BIOCHEM_FORCE_WALL_MU0 was harmful; that override
# was removed from gnode_biochem.py — ForceWallMu0 leg no longer exists.
#
#   .\scripts\run_biochem_wall_mu0_ab_10min.ps1
#   .\scripts\run_biochem_wall_mu0_ab_10min.ps1 -DryRun
#
param(
    [ValidateSet("LearnWallMu")]
    [string] $Variant = "LearnWallMu",
    [int] $TeacherEpochs = 8,
    [int] $ValEvery = 2,
...
$SummaryPath = Join-Path $RepoRoot "outputs\reports\training\biochem\wall_mu0_ab_10min_summary.txt"
...
Write-Host "Wall learning probe started | epochs=$TeacherEpochs | warm_start=$UseWarmStart" -ForegroundColor Cyan
...
function Set-CommonFairEnv {
    ...
    # Best balanced architecture from fair h2h (geometry-isolated wall head).
...
foreach ($leg in $legs) {
    ...
    $runNote = "wall_learn_probe_ep${TeacherEpochs}_${hostName}"
...
Write-Host ""
Write-Host "Wall learning probe complete | batch duration=${batchMins}m (${batchSecs}s)" -ForegroundColor Green
