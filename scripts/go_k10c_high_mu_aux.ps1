# K10c: K10b forward stack + step-2 data-only backprop with high-μ log aux (no LOSS_ISOLATE).
# Prereq: outputs/kinematics/kinematics_best.pth. Delete outputs/biochem/*.pth before fresh run.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\go_k10c_high_mu_aux.ps1"
...
$env:BIOCHEM_MU_ADDITIVE_DELTA = "1"
Remove-Item Env:BIOCHEM_USE_WALL_DELTA_HEAD,Env:BIOCHEM_LOSS_ISOLATE -ErrorAction SilentlyContinue
...
$env:BIOCHEM_RUN_NOTE = "K10c_high_mu_aux"
...
if ($Fresh) {
    Remove-Item (Join-Path $RepoRoot "outputs\biochem\*.pth") -ErrorAction SilentlyContinue
    Write-Host "Fresh K10c: deleted outputs/biochem/*.pth (kept kinematics_best.pth)." -ForegroundColor Cyan
}

python -m src.training.train_biochem_corrector --new --epochs 12 --save-best --run-name K10c_high_mu_aux
