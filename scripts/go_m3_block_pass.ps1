...
#   powershell ... -MinPass       # same recipe, min epochs for check_m3_align_gate (~1-1.5h)
...
$Ramp1Epochs = if ($Probe) { 2 } elseif ($MinPass) { 3 } elseif ($Turbo) { 3 } else { 3 }
$Ramp2Epochs = if ($Probe) { 2 } elseif ($MinPass) { 3 } elseif ($Turbo) { 4 } else { 6 }
$AlignEpochs = if ($Probe) { 3 } elseif ($MinPass) { 6 } elseif ($Turbo) { 6 } else { 12 }
...
$mode = if ($Probe) { "PROBE (trends only, ~30-45m)" } elseif ($MinPass) { "MIN-PASS (gate min epochs, ~1-1.5h)" } elseif ($Turbo) { "TURBO full block (~3-5h)" } else { "FULL block (~8-12h)" }
...
$alignNote = if ($Probe) { "${AlignRunNote}_probe" } elseif ($MinPass) { "${AlignRunNote}_minpass" } else { $AlignRunNote }
...
if ($Probe) {
    $gateRc = Invoke-PythonRc scripts/check_m3_align_gate.py --run-note $alignNote --min-epochs 3 -Quiet
    if ($gateRc -eq 0) {
        Write-Host "[OK] M3 probe: trend gate passed (--min-epochs 3)" -ForegroundColor Green
    } else {
        Write-Host "[WARN] M3 probe: trend gate not passed; inspect run.jsonl" -ForegroundColor Yellow
    }
    Write-Host "[i]  python scripts/check_m3_align_gate.py --run-note $alignNote --min-epochs 3" -ForegroundColor Cyan
    Write-Host "[i]  Min formal pass: go_m3_block_pass.ps1 -MinPass" -ForegroundColor Cyan
    exit 0
}

if ($MinPass) {
    $gateRc = Invoke-PythonRc scripts/check_m3_align_gate.py --run-note $alignNote --min-epochs 6 -Quiet
    if ($gateRc -eq 0) {
        Write-Host "[OK] M3 min-pass: check_m3_align_gate passed (6 val epochs)" -ForegroundColor Green
        exit 0
    }
    Write-Host "[ERR] M3 min-pass: align gate failed" -ForegroundColor Red
