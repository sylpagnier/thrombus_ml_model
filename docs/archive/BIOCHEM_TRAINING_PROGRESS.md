...
> **Archive status (2026-07):** The GNODE teacher/corrector path (`train_biochem_corrector.py`) and its launchers were removed from the active script surface. Low-rank kine adapters (formerly "LoRA") were removed from the codebase on 2026-07-21. This document remains as a historical run chronicle. **Active stack:** `biochem_gnn` / `go_biochem_gnn.ps1` / mat-growth — see `AGENTS.md` and `scripts/README.md`.
...
| **3** | Full multitask (**deprecated** default) | Kendall sum: PDE + walls + ADR + data heads (not data-only) | Full corrector (retired) | `BIOCHEM_COMPLEXITY_STEP=3` — **regressed** vs step-2; use only with `BIOCHEM_LEGACY_LOSSES=1` |
...
| Piece | Choice |
|-------|--------|
| Flow | **RGP-DEQ predicted kine** (`BIOCHEM_GT_KINE_VEL=0`), Stage-A `kinematics_best.pth` inside macro loop |
...
| **Mu6h comprehensive sweep** (pred kine, teacher+corrector+synth) | Best val mu all **~0.44**; high-mu **~1.09**; step-3 regresses | **Partial** | **§173**: retired `go_mu_complexity_6h.ps1`; init Lane A promoted; **FULL_step2** all **0.446** (corr ep17) high **1.096**; **FULL_overnight** teacher **0.444** ep10 then **OOM** corr ep40; step-3 **0.459**; r **~-0.02**; finish gate PASS (plumbing) |
...
- **Mu6h sweep (§173)**: pred-kine **teacher+corrector+synth** stack **plumbing PASS**; best completed mu **~0.446** (`FULL_step2` corrector); overnight teacher **0.444** @ ep10 but **OOM** @ corrector ep40 (`mu_ratio~22`, TBPTT=14 on 4GB). **Step-2 beats step-2.5 and step-3** on this init; corrector adds **~0.003** vs teacher-only. (Retired GNODE path; see archive note at top.)
...
### 87. K0 viz (`health10h` / `K0_carreau_kinematic`): weak flow is **biochem rollout path**, not standalone RGP-DEQ (2026-05-25)
...
- **Fair kinematics comparison**: same run also opens **`Kinematics (RGP-DEQ), steady — patient007`** (`kinematics_best.pth`, one-shot on biochem mesh). If that window shows healthy **|u|~1+** while temporal slider stays weak, the bug is **biochem coupled macro rollout** (DEQ + `mu_encoder` feedback + resting species IC + low-shear Carreau lockup), **not** the kinematics backbone weights.
- **Readout**: K0 **does not** clear the “healthy kin baseline” hypothesis on **biochem rollout**; it only shows clot architecture is off in **`μ_eff`**. Next compare **steady RGP-DEQ figure** vs **Biochem `μ_eff` channel** at final time (Figure 2 dynamic viscosity), then viz **S0** (best scoreboard leg).
...
- **Forward vs display viscosity (S0)**: training uses **`BIOCHEM_MU_SIMPLE_LOG_RESIDUAL=1`** → **`μ_eff = μ_kin × exp(Δlogμ)`**, **no** explicit μ₁/μ₂ in forward. Temporal slider rows 4–6 and Figure 2 **recompute** COMSOL-style **`μ_b×(1+μ₁+μ₂)`** via `_biochem_rheology_fields` on rolled-out species — **overstates** clot when FI explodes open-loop (§85). For S0, also inspect **stored `mu_eff` ND channel** in the rollout tensor (Figure 1 steady kin panel uses RGP-DEQ, not this channel).
- **Steady RGP-DEQ (same run)**: **|u|** inlet core **~1.5–1.7** ND but **mostly dark downstream**; **μ_eff** lumen core **elevated** (yellow ~5–8 ND) — **better than biochem rollout** at all slider times, **not** a perfect COMSOL match. Confirms **kin backbone partially OK**, **coupled rollout + chemistry integration** is the failure mode.
...
- **Steady RGP-DEQ** (same session): **|u| ~1.4–1.6** at inlet, structured lumen — **closer to COMSOL** than biochem **t=0** one macro step. Fair read: **kinematics_best weights OK**; gap is **biochem coupled macro solve** (Anderson + `mu_encoder` @ t=0, then Carreau feedback), not missing SIREN/width load.
...
- **Init:** `gnode12_lane_a_promoted/biochem_teacher_best_high_mu.pth` (delta-μ, pred kine, `mu_ratio_max=20`); stack = RGP-DEQ + GNODE Phase3, LoRA, pseudo bank **9/9**.
...
| 2026-05-25 | **health10h** `S0_simple_residual` (`MU_LOG`, `SIMPLE_LOG_RESIDUAL`, 22ep warm) | **0.451** (ep20) | **1.743** | **0.346** viz t0 \|u\| | high **1.12** | **Best viz_health 13.37**; **viz patient007**: t0 rollout **>K0**; slider1 **FI/μ₂ global flood**, **u→0** (§88); steady RGP-DEQ **partial** |
...
| 2026-05-25 | **K0_stage_a_parity_fresh** viz (patient007, `biochem_teacher_best_high_mu.pth`) | **1.471** (ckpt ep0 tag) | **2.050** | **—** | high **1.232** | **§89**: t0 biochem **\|u\|~1.0–1.2** vs COMSOL **~1.5–2**; steady RGP-DEQ **~1.6**; **t≈9540** narrow **jet**; **μ₂=80** diag-only; **μ_eff** row black — open-loop ODE + macro re-DEQ |
