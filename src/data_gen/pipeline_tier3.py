"""
Tier 3 data generation: synthetic Gmsh cohort + Tier 3 graphs, and/or patient COMSOL extraction.

Synthetic track: ``VesselGeneratorTier3`` -> ``MeshToGraphTier3`` (``data/raw/tier3`` -> ``graphs_tier3``).

Patient track: ``PatientDataExtractor`` (``data/raw/tier3_patients`` + COMSOL exports -> ``graphs_tier3_patients``).
"""
...
def run_interactive_pipeline() -> None:
    print("\n=== Tier 3 data generation (synthetic / patient extraction) ===\n")
    ...
    if run_syn:
        print("\n--- Synthetic Tier 3 (Gmsh -> graphs_tier3) ---\n")
        ...
        run_m2g = _prompt_yes_no(
            "Run MeshToGraphTier3 (synthetic .msh -> graphs_tier3 .pt, replaces *.pt in output dir)?",
            default=True,
        )
        if run_m2g:
            max_files = _prompt_optional_max_files()
            print("\n--- MeshToGraphTier3 ---\n")
            MeshToGraphTier3().run(max_files=max_files)

    if run_pat:
        print("\n--- Patient / anchor extraction (tier3_patients) ---\n")
        print(
            "Expect COMSOL text exports + meshes under data/raw/tier3_patients "
        ...
        if not _prompt_yes_no("Run PatientDataExtractor now?", default=True):
            ...
            PatientDataExtractor(tier="tier3_patients").run()

    print("\n=== Tier 3 pipeline finished ===\n")


def _parse_batch_args(argv: list[str]) -> Optional[argparse.Namespace]:
    p = argparse.ArgumentParser(description="Tier 3 data pipeline (synthetic and/or patient extract).")
    ...
    p.add_argument("--skip-mesh", action="store_true", help="Synthetic: skip MeshToGraphTier3.")
    p.add_argument("--max-mesh-files", type=int, default=None, help="Synthetic: cap meshes for MeshToGraphTier3.")
...
def run_batch_pipeline(args: argparse.Namespace) -> None:
    if args.track in ("synthetic", "both"):
        if not args.skip_vessel:
            ...
            print(f"--- VesselGeneratorTier3: n={args.num_vessels} level={args.level} start={start_idx} ---\n")
            vg.run_pipeline(
        ...
        if not args.skip_mesh:
            print("--- MeshToGraphTier3 ---\n")
            MeshToGraphTier3().run(max_files=args.max_mesh_files)
