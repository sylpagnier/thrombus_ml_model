"""Manual DEQ / Anderson sanity checks (opens matplotlib windows). Not part of pytest.

Run: ``python -m src.tools.verify_deq_convergence``
"""
...
def test_visualize_convergence_audit():
    ...
    for idx, rheology in enumerate(["newtonian", "carreau"]):
        ax = axes[idx]
        ...
        ax.set_yscale('log')
        ax.set_title(f"Convergence Audit: {rheology.upper()}")
        ax.legend()
