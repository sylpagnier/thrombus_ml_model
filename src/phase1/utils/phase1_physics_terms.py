"""
Single source of truth for Phase 1 (Tier 1 / Tier 2) physics loss **terms** given a state
tensor ``pred`` and graph ``data``.

``train_tier1.compute_step_loss`` and ``train_tier2.compute_step_loss`` call this so validation
tests exercise the **exact** same code path as training (no duplicated derivative stacks).
"""
...
from src.phase1.utils.anchor_mask import anchor_node_mask
...
def compute_phase1_physics_terms(
    pred: torch.Tensor,
    data,
