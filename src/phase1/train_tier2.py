...
def validate_and_plot(model, val_data, epoch, device, save_name="val_epoch"):
    ...
    with torch.no_grad():
        ...
        # Plotting the predicted effective viscosity (mu) for Tier 2 insights
        mu_pred = pred[:, 3].cpu().numpy()

    plt.figure(figsize=(10, 4))
    sc = plt.scatter(coords[:, 0], coords[:, 1], c=mu_pred, cmap='viridis', s=5)
    plt.colorbar(sc, label="Predicted ND-Viscosity ($\mu$)")
    plt.title(f"Tier 2 Validation (Carreau-Yasuda) - Epoch {epoch}")
    plt.axis('equal')

    save_dir = Path("reports/figures/tier2")
...
def quantify_performance(model, val_loader, kernels, device):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            # 4. Rheology Error
            rheo_err = kernels.rheology_loss(pred, data, props)
            metrics["rheology"].append(rheo_err.item())
...
def load_dataset():
    ...
    # Updated to point to a distinct tier 2 folder
    data_dir = current_script_dir.parent.parent / "data" / "processed" / "graphs_tier2"
...
def train_tier2(epochs=50, lr=1e-4, warm_up_epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(pbar):
            ...
            if hasattr(data, 'is_anchor'):
                ...
                if node_is_anchor.sum() > 0:
                    # Train all 4 channels against ground truth
        ...
        # --- Step Scheduler & Post-Epoch Evaluation ---
        scheduler.step()
