    ...
    def _sync_overlap_for_current_tier():
        if restrict_to_overlap:
            ...
            state["overlap_indices"] = sorted(
                idx for idx in (_extract_vessel_idx(p) for p in graph_dir.glob("vessel_*.pt"))
                if idx is not None
            )
...
def _pick_overlap_filename_interactively(overlap_indices):
    if len(overlap_indices) == 0:
        print("No overlapping vessels found to inspect.")
        return None

    overlap_lookup = set(overlap_indices)
    print("\nOverlap vessel IDs:")
    print(", ".join(str(v) for v in overlap_indices))

    while True:
        raw = input("\nType vessel number to inspect (for example: 73), or q to quit: ").strip().lower()
        if raw in ("q", "quit", "exit", ""):
            return None
        try:
            vessel_id = int(raw)
            if vessel_id in overlap_lookup:
                return f"vessel_{vessel_id}.pt"
            print(f"Vessel {vessel_id} is not in overlap set. Pick one from the listed IDs.")
        except ValueError:
            print("Invalid input. Enter a vessel number.")
...
def _prompt_tier(default="tier1"):
    while True:
        raw = _prompt_text("Tier", default)
        tier = _normalize_tier_value(raw)
        if tier is not None:
            return tier
        print("Invalid tier. Use: tier1 or tier2 (also accepts 1/2).")
...
def _is_valid_comsol_result(npz_path):
    ...
    try:
        with np.load(npz_path) as data_npz:
            ...
            # Guard against trivial zero fields in corrupted/replaced exports.
...
def list_indices_with_valid_comsol_results(
    ...
    if verbose and emit:
        print(f"\n{'=' * 60}")
        print(f" VALID PHASE1 + COMSOL OVERLAP ({tier.upper()})")
        print(f"{'=' * 60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inspect Phase 1/2 graph samples")
    parser.add_argument(
        "--filename",
        type=str,
        default=None,
        help="Graph filename (for example: vessel_73.pt). If omitted, you'll be prompted to pick one.",
    )
    parser.add_argument(
        "--tier",
        type=str,
        default=None,
        help="Tier used to resolve default graph output directory",
    )
    parser.add_argument(
        "--proc-dir",
        type=str,
        default=None,
        help="Optional directory containing graph .pt files",
    )
    parser.add_argument(
        "--cfd-dir",
        type=str,
        default=None,
        help="Optional directory containing COMSOL .npz CFD files",
    )
    parser.add_argument(
        "--list-cfd-overlap",
        action="store_true",
        help="Print overlap count for vessel indices present in graph data and valid COMSOL CFD outputs",
    )
    parser.add_argument(
        "--inspect-sample",
        action="store_true",
        help="Inspect a single graph sample (interactive picker when --filename is omitted)",
    )
    parser.add_argument(
        "--verbose-overlap",
        action="store_true",
        help="Print full overlap diagnostics and index list (default prints overlap count only)",
    )
