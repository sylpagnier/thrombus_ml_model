"""Interactive anchor CFD inspector and batch health scan.

Run as a normal Python script (not via pytest) so ``__main__`` executes::

    .venv\\Scripts\\python.exe src/phase1/tests/inspect_anchor.py --tier tier2 --scan-only

Pytest only discovers ``test_*`` functions; inventory unit tests live in
``test_data_gen_inventory.py``.
"""
import argparse
import csv
import random
...
def inspect_all_anchors(active_tier="tier1", export_csv=True):
    ...
    if export_csv and len(rows) > 0:
        out_path = get_project_root() / "reports" / f"{active_tier}_anchor_health.csv"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        with open(out_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                [
                    "sample_idx", "ok", "reason", "nodes", "vel_min", "vel_max", "vel_mean",
                    "p_min", "p_max", "p_std", "u_abs_max", "has_mu", "mu_min", "mu_max",
                    "nan_ratio", "quality_flags",
                ]
            )
            for r in rows:
                writer.writerow(
                    [
                        r.get("sample_idx"), r.get("ok"), r.get("reason"),
                        r.get("nodes"), r.get("vel_min"), r.get("vel_max"), r.get("vel_mean"),
                        r.get("p_min"), r.get("p_max"), r.get("p_std"), r.get("u_abs_max"),
                        r.get("has_mu"), r.get("mu_min"), r.get("mu_max"),
                        r.get("nan_ratio"), "|".join(r.get("quality_flags", [])),
                    ]
                )
    ...
    while True:
        ...
        try:
            ...
            if enable_regenerate:
                ...
                fig._regen_btn = regen_btn  # keep button alive
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Tier used to resolve default output directory (tier1 or tier2).",
    )
    parser.add_argument(
        "--sample-idx",
        type=int,
        default=None,
        help="Optional sample index to plot. If omitted, a random sample is chosen.",
    )
    parser.add_argument(
        "--scan-all",
        action="store_true",
    ...
    parser.add_argument(
        "--scan-only",
        action="store_true",
        help="Run the full-anchor health scan and CSV export, then exit (no matplotlib).",
    )
    parser.add_argument(
        "--no-export-csv",
        action="store_true",
        help="Disable CSV export for scan summary.",
    )
    ...
    # Always scan all anchors first (quantitative health + optional CSV).
