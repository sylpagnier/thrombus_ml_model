...
from src.phase1.utils.anchor_mask import graph_has_anchor, anchor_node_mask
...
def train_tier1(epochs=60, lr=1e-4, warm_up_epochs=10, adam_epochs=60):
    ...
    best_physics_path = model_dir / "tier1_best_physics.pth"
    ...
    if resume_enabled and latest_ckpt_path.exists():
        ...
        if start_epoch < adam_epochs and ckpt_optimizer_type == "AdamW":
            ...
            print("ℹ️ Ignoring legacy LBFGS checkpoint state; continuing with AdamW.")
    ...
    hard_csv_path = get_project_root() / "reports" / "tier1_anchor_hardness.csv"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if should_save_ckpt:
            ...
            print(f"💾 Saved Tier 1 checkpoint -> {latest_ckpt_path.name} (every {ckpt_every} epoch(s))")
