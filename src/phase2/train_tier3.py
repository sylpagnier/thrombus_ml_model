...
def _ease01(t: float, easing: str) -> float:
    """Map [0,1] progress with optional smoothing (less abrupt curriculum than linear)."""
...
def _tier3_metrics_jsonl_path():
    return get_project_root() / "reports" / "tier3_metrics.jsonl"
...
def _tier3_debug_log_path():
    return get_project_root() / "reports" / "tier3_debug.log"


def _tier3_dbg_line(msg: str) -> None:
    """Stdout + append to ``reports/tier3_debug.log`` (tqdm often obscures raw prints)."""
...
def remap_tier2_encoder_weight_to_tier3(
...
def train_tier3(epochs=25, lr=1e-3):
    ...
    tier3_resume_path = model_dir / "tier3_best_bio.pth"
    ...
    if resume_enabled and tier3_resume_path.exists():
        ...
        if 'kin_encoder.0.weight' in mapped_state_dict:
            ...
            if tier2_weight.shape[1] != model_weight.shape[1]:
                ...
                new_weight = remap_tier2_encoder_weight_to_tier3(tier2_weight, model_weight)
    ...
    train_anchor_dataset = PatientDataset(root=str(get_project_root()), file_list=train_anchors)
    val_anchor_dataset = PatientDataset(
        root=str(get_project_root()),
        file_list=val_anchors if len(val_anchors) > 0 else train_anchors
    )
    train_synth_dataset = PatientDataset(root=str(get_project_root()), file_list=train_physics)
    ...
    latest_ckpt_path = model_dir / "tier3_latest_checkpoint.pth"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch % 2 == 0:
            ...
            viz_dir = get_project_root() / "reports" / "tier3_val_viz"
