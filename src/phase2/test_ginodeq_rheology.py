...
class TestDualTriggerRheology(unittest.TestCase):
    ...
    def test_rheology_thresholds(self):
        ...
        """
        # COMSOL Step 3 (mu1): Location 2E7, Max 80
        self.assertEqual(self.model.mat_crit, 2e7)
        # COMSOL Step 5 (mu2): Location 0.6, Max 80
        self.assertEqual(self.model.fi_crit, 0.6)
        self.assertEqual(self.model.mu_ratio_max, 80.0)

    def test_mu1_sigmoid_platelet_logic(self):
        """
        Test the soft step-function for Platelet-driven viscosity (mu1).
        It should be ~0 well below the 2e7 threshold, and ~80 well above it.
        """
        ...
        # Soft sigmoid at exactly threshold should equal max / 2 (80 / 2 = 40)
        self.assertAlmostEqual(mu1_thresh, 40.0, places=2)

        # Assert limits bounded by mu_ratio_max
        self.assertTrue(mu1_low < 1.0, "Viscosity multiplier should be near 0 at low Mat")
        self.assertTrue(mu1_high > 79.0, "Viscosity multiplier should be near 80 at high Mat")

    def test_mu2_sigmoid_fibrin_logic(self):
        """
        Test the soft step-function for Fibrin-driven viscosity (mu2).
        It should be ~0 well below the 0.6 threshold, and ~80 well above it.
        """
        ...
        # Soft sigmoid at exactly threshold should equal max / 2 (80 / 2 = 40)
        self.assertAlmostEqual(mu2_thresh, 40.0, places=2)

        # Assert limits
        self.assertTrue(mu2_low < 1.0, "Viscosity multiplier should be near 0 at low Fibrin")
        self.assertTrue(mu2_high > 79.0, "Viscosity multiplier should be near 80 at high Fibrin")
