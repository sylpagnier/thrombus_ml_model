...
def run_convergence_audit(max_iters=30):
    ...
    # NEW: The GlobalMixingBlock requires a batch vector
    # Since this is 1 graph, everyone belongs to batch "0"
    ...
    with torch.no_grad():
        ...
        print(f"Starting Fixed-Point Iteration (Max Iters: {max_iters})...")
        start_time = time.time()

        # Iterative loop
        for i in range(max_iters):
            # NEW: We must pass 'data.batch' to the core
            z_next = model.core(z, data.edge_index, edge_attr, data.batch)
    ...
    plt.title('rGINO-DEQ Fixed-Point Convergence Audit (with Global Mixing)')
